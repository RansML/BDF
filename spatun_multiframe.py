...
from utils_data_preprocessing.train_test_split import split_n_save
...
def plot():
    ...
    plotter = BHM_PLOTTER(args=args, plot_title=args.plot_title, surface_threshold=args.surface_threshold, variance_threshold=args.variance_threshold, query_dist=args.query_dist, occupancy_plot_type=args.occupancy_plot_type,
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", '--start-n', type=int, default=0, help='')
    parser.add_argument("-e", '--end-n', type=int, default=0, help='')
    ...
    @timeout_decorator.timeout(1800) #in seconds
    def tqp_methods():
    ...
    base_filebase = "/".join(args.dataset_path.split("/")[:-1]) + "/{:06d}"
    train_filebase = base_filebase + "_train"
    eval_filebase = "/".join(args.eval_path.split("/")[:-1]) + "/{:06d}_test"
    plot_title_base = args.plot_title + "_{:06d}"
    save_model_path_base = args.save_model_path + "_{:06d}"
    save_query_data_path = args.save_query_data_path + "_{:06d}"

    # file_idxs = range(10)
    file_idxs = range(args.start_n, args.end_n)
    for file_idx in file_idxs:
        try:
            fn_train = train_filebase.format(file_idx)
            args.eval_path = eval_filebase.format(file_idx)
            args.plot_title = plot_title_base.format(file_idx)
            args.save_model_path = save_model_path_base.format(file_idx)
            args.save_query_data_path = save_query_data_path.format(file_idx)
            if not os.path.isfile(fn_train):
                base_file = base_filebase.format(file_idx)
                print(f"Splitting {base_file} into train and test sets...")
                split_n_save(base_file, 0.2)


            cell_resolution = args.hinge_dist
            args.report_notes = 'file_idx={}; hinge_dist={}; gamma={}'.format(file_idx, args.hinge_dist, args.gamma)
            print('\n=============================================')
            ...
            try:
                ...
                exception_message_lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
                print("".join("\t > " + line for line in exception_message_lines))

                print(" Rans Error TIMEOUT. Trying rest of the hinge factors aborted!")
                timeout_err = True
                exit()
            ...
            print(" Rans Error EXCEPTED when fi={}, gi={}".format(fi, gi))
